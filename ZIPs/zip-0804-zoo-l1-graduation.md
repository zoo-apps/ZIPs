...
This proposal graduates the Zoo chain from a Lux L2 application (Chain ID 200200, settling state commitments to Lux C-Chain) to a sovereign Lux L1 with its own validator set, Quasar 3.0 triple-consensus finality (BLS + Corona + ML-DSA per LP-020), and independent economic security backed by `$ZOO` and `$AI` staking. The graduation follows the Quasar-Native App Stack pattern (LP-133) and embeds Zoo L1 in the Lux chain topology (LP-134) alongside P/C/X/Q/Z/A/B/M/F-Chains. Activation: 2025-12-25.
...
- Triple-consensus per LP-020: BLS12-381 (classical fast path), Corona (post-quantum threshold), ML-DSA-65 (post-quantum identity).
...
| External chain | Role |
|---|---|
| Q-Chain (QVM) | Corona DKG ceremonies for Zoo validator key material |
