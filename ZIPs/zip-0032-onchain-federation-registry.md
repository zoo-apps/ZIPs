...
- **Liquidity blocklist**: the Zoo subnet's federation precompile MUST reject registrations with `brandId == bytes32("liquidity")` (same hardcoded blocklist constant as HIP-0304); Liquidity's federation registry lives on its own external EVM and is not bridged into Zoo.
